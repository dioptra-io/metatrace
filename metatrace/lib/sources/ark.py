...
class ArkTeamProbingFile:
    """
    >>> ArkTeamProbingFile.from_filename("daily.l7.t1.c000027.20070913.amw-us.warts.gz").filename
    'daily.l7.t1.c000027.20070913.amw-us.warts.gz'
    >>> ArkTeamProbingFile.from_filename("aal-dk.team-probing.c007146.20190103.warts.gz").filename
    'aal-dk.team-probing.c007146.20190103.warts.gz'
    """
    ...
    def filename(self) -> str:
        if self.team:
            return f"daily.l7.t{self.team}.c{self.cycle:06}.{self.date:%Y%m%d}.{self.monitor}.warts.gz"
        return (
            f"{self.monitor}.team-probing.c{self.cycle:06}.{self.date:%Y%m%d}.warts.gz"
        )
    ...
    def from_filename(
        ...
        elif m := re.match(r".+\.t(\d)\.c(\d+)\.(\d+)\.([\w-]+)\.warts\.gz", name):
            team, cycle, date, monitor = m.groups()
            return cls(
                monitor=monitor,
                cycle=int(cycle),
                date=datetime.strptime(date, "%Y%m%d").replace(tzinfo=timezone.utc),
...
def ark_probe_data_list(
    ...
    """
    >>> ark_probe_data_list(1, datetime(2020, 1, 1), datetime(2020, 1, 2))
    895
    """
    files = []
    ...
    with httpx.Client(base_url=base_url) as client:
        for date in time_range:
            ...
            for link in ark_get_links(client, path):
                if file := ArkTeamProbingFile.from_filename(link, md5s.get(link)):
                    files.append(file)
    return files
